...
object DiscordPresenceManager {
    ...
    // We intentionally store the application Context (or whatever the caller passed) â€” callers
        ...
        rpcMutex.withLock {
            try {
                ...
                if (result.isSuccess) {
                    ...
                    consecutiveFailures++
                    Timber.tag(logTag).w("updatePresence failed silently â€” updateSong returned failure (consecutive=%d)", consecutiveFailures)
                    false
                }
            } catch (ex: Exception) {
                consecutiveFailures++
        ...
        job = scope!!.launch {
            ...
            while (isActive) {
                try {
                    ...
                    Timber.tag(logTag).e(e, "loop error â†’ ${e.message}")
                }

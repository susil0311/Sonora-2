...
object KuGou {
    ...
    private fun normalizeTitle(title: String) =
        title.replace("\\(.*\\)".toRegex(), "").replace("ï¼ˆ.*ï¼‰".toRegex(), "")
            .replace("ã€Œ.*ã€".toRegex(), "").replace("ã€Ž.*ã€".toRegex(), "")
            .replace("<.*>".toRegex(), "").replace("ã€Š.*ã€‹".toRegex(), "")
            .replace("ã€ˆ.*ã€‰".toRegex(), "").replace("ï¼œ.*ï¼ž".toRegex(), "")

    private fun normalizeArtist(artist: String) =
        artist.replace(", ", "ã€").replace(" & ", "ã€").replace(".", "").replace("å’Œ", "ã€")
            .replace("\\(.*\\)".toRegex(), "").replace("ï¼ˆ.*ï¼‰".toRegex(), "")
    ...
    private val BANNED_REGEX = ".+].+[:ï¼š].+".toRegex()
